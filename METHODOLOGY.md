...
- **Part IV — Tool Catalog and Functional Test Procedures (PheronAgent Case Study)** (source: `tool_testing_protocol.md` + `tool_testing_procedures.md`)
  Categorical test methods and concrete L3-TOOL test scenarios for 72 native/external MCP tools — not part of the universal battery, PheronAgent-specific. 18 were moved to Part II Section 13 (SUPP-TOOL-05 was never assigned); as of 2026-09-05, all 72 UBIDs have at least one verified test block — see the corrected Part VII.4/VIII.4 coverage note (the previous "10 remain a genuine coverage gap" claim was itself stale/partially incorrect even before this correction; several of the 10 it listed were already covered).
...
> **Role:** This is the project's **single canonical test protocol** (see the Part VIII.3 consolidation decision). The core universal battery (66 test blocks as of the Version 9 correction, up from the original 58 — see Version 9 changelog entry for the 8 blocks added 2026-07-25 that were never previously recorded here) + the 42 SUPP-TOOL-20..62 blocks added in this document (Section 13) make up a **total of 108 unique test blocks**. (Section 13 also lists SUPP-TOOL-01..04/06..19 as cross-references to Part IV.b's L3-TOOL-01..04/06..19 — these are the same 18 scenarios under a second ID, not additional blocks; SUPP-TOOL-05 was never assigned; see the Version 8 changelog entry.) It includes the 4-layer architecture, golden dataset schema, acceptance/rejection taxonomy, CI integration, and certification template. Sections 1–12 are carried over below as-is (with the original heading hierarchy shifted two levels in); Section 13 is an addendum specific to this document.
>
...
STEP 2: Same harness, same 108 blocks
...
| Run (108 blocks, same harness) | 🔶 **PENDING EXECUTION** |
...
Prompt:           "read the file /Users/<user>/Desktop/nonexistent_file_xyz.txt"
...
| N-07 | "count the swift files in the PheronAgent directory" | 32 (shell_exec) | CALL(32) |
...
> **Scope warning (Version 5):** Unlike the universal battery in Part II Sections 4-10, this part is a case study specific to **PheronAgent's own tool catalog** — the 72 tools tested here (Blender, Xcode, WhatsApp, Stripe integration, etc.) are not generic capabilities every agent should have. It is not a direct template for someone building a different agent, but it is useful as an example of "how a tool catalog is documented/tested in a real agent."
> **Role:** Contains the category-based test methods and concrete L3-TOOL scenarios for Pheron Agent's 72 native and external MCP tools (multimedia/hardware, web/browser, communication/calendar, file/developer, external integrations). **Update (post-harmonization):** The 18 concrete scenarios below (IV.b, IDs 01-04/06-19 — 05 was never assigned) are now also available converted into the official test-block format (SUPP-TOOL-01..04/06..19) in Part II Section 13 — they are kept here in their original source format for reference. **Update (Version 9, 2026-09-05):** the previously-listed "remaining 10 UBIDs with no concrete scenario" (21, 22, 37, 49, 96, 97, 98, 99, 102, 104) claim was audited against the actual golden dataset and found to be substantially wrong even before this correction — UBID 21 (`calendarEvents`) no longer exists in the codebase (removed, confirmed dead code), and 22/37/49/96/97/98/99/102/104 already had real test blocks (EK-TOOL-21/22/23/24/25/26/27/28/29 and others). As of this version, all 72 UBIDs have at least one verified test block — see Part VII.4/VIII.4.

## IV.a — Test Procedures (Category-Based Methodology)

### AI Agent Tool Testing Protocol

This protocol contains the concrete test scenarios and expected tool-call templates to be used to verify all capabilities of all of Pheron Agent's 72 tools.
...
*   **Prompt:** `"do the music DNA analysis on the file '/Users/<user>/Local Documents/Suno Downloads/Aura di Luce (Aura of Light - Işık Hale)/Aura di Luce (Aura of Light - Işık Hale) (1).mp3' and tell me the music genre"`
...
*   **Prompt:** `"auto-fill the ID3 tags of the MP3 files in the directory /Users/<user>/Local Documents/Suno Downloads/Aura di Luce (Aura of Light - Işık Hale) using the txt and jpeg files, overriding the TPE1 value to 'Aura Artist' and the TALB value to 'Aura Album'"`
*   **Expected Tool:** `id3_processor` (UBID: 85)
*   **Criterion:** `CALL(85) WITH {"directory": "/Users/<user>/Local Documents/Suno Downloads/Aura di Luce (Aura of Light - Işık Hale)", "custom_tags": {"TPE1": "Aura Artist", "TALB": "Aura Album"}}` must fire.
...
1. **Local File System Dependency (Lack of Isolation):** Running the L3-TOOL-19 (ID3 Music Tag Processor, UBID: 85) and L1-FILE-01 (write_file, UBID: 34) tests from `tool_testing_protocol.md` directly in a local host directory such as `/Users/<user>/` eliminates test safety and causes side effects.
...
**Update (Version 9, 2026-09-05) — this section's own bookkeeping had become internally contradictory; both the contradiction and the correction are recorded here rather than silently rewriting the numbers above:**

- The "UBIDs left out of coverage" row directly above claims **0**, closed by SUPP-TOOL-20..29 — but until this version, this document's own front matter and the Part IV role box both still said **"10 remain a genuine coverage gap."** Both could not be true at once; this document carried that internal contradiction for some time.
- Independently re-verified from scratch on 2026-09-05, against the current codebase (`ToolIDs.swift`'s `ToolUBID` enum) and the current golden dataset (`golden_dataset_126.json`, EliteAgent private repo — the source of truth this document's case study is built on): **all 72 current `ToolUBID` cases have at least one real test block.** Verified by cross-referencing every case's actual registered tool name against `expected.ubid` + `expected.tool` + `expected.tools_order` + `require_tool_called` across every dataset block — a keyword search alone produces both false positives (e.g. "arka planda" [run in the background] matching "arka plan" [photo background] by substring) and false negatives (e.g. `tools_order` entries and reference-implementation cross-links that a naive `expected.ubid`-only scan misses).
- Several individual claims in Part VII.4 above and the old Part IV role box were already wrong, independent of anything changed this session: UBID 21 (`calendarEvents`) no longer exists in the codebase at all (removed as dead code — confirmed via `ToolIDs.swift`'s own comment on the deletion); UBID 85 (`id3_processor`) and UBIDs 17/37 (`whatsappMessage`/`messengerMessage`), both listed above as "never tested," were in fact already tested (via L3-TOOL-19 and L3-TOOL-09/EK-TOOL-22 respectively) at the time this section was last edited.
- **Part VII.4's list and the table above are left as-written** — a dated record of what earlier passes believed, consistent with this document's own stated practice (see the Version 8 changelog entry's identical rationale). This note records the current, independently-verified state as of 2026-09-05; it does not silently rewrite the history above it.
- A full line-by-line reconciliation of Part VII/VIII's historical UBID bookkeeping — which figure was correct when, and exactly where each later one drifted — was **not** attempted in this pass. Doing so would mean reconstructing intent across at least four prior versions, with real risk of introducing a new, confidently-wrong number in the process of trying to fix an old one. Flagged here as a known, unresolved follow-up, not silently closed.
