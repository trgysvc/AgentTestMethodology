...
RESULTS = "/Users/<user>/Developer/PheronAgent/Tests/AgentTestSuite/results/results_72_remaining_k5.jsonl"
LOG = "/Users/<user>/Developer/PheronAgent/Tests/AgentTestSuite/results/run_72_remaining_k5.log"
HEALTH = "http://localhost:11500/api/health"
AGENT = "http://localhost:11500/api/agent"
AUDIT = "/Users/<user>/Library/Logs/PheronAgent/audit.log"
...
# v47: 280s was shorter than several genuinely-slow real tasks (audio analysis ~15min, Blender
# render, Xcode build ~15-20min) — confirmed via audit.log timestamp correlation across
# L3-TOOL-01/L3-TOOL-12/L3-TOOL-13/L3-ROUTE-01's DATA-CORRUPT (fully empty) records in the
# 434-postfix batch: each empty record's save gap from the prior test matched 3 wasted
# 280s-timeout retry cycles (~14-16min) exactly, while runs that genuinely finished fast (quick
# error/clarify) returned well under 280s. Raised to match wait_until_free's own 1200s budget so
# a real slow task gets one honest attempt instead of being torn down by an under-sized client
# timeout and silently recorded as empty.
def curl_agent(prompt, history=None, timeout=1200):
    body = {"prompt": prompt}
    if history:
...
SIMPLE_TESTS = [
    ...
    ("HR-02", "/Users/<user>/Desktop/olmayan_dosya_xyz.txt dosyasını oku"),
    ...
    ("L3-ROUTE-02", "merhaba"),
    ("L3-TOOL-01", "'/Users/<user>/Local Documents/Suno Downloads/Aura di Luce (Aura of Light - Işık Hale)/Aura di Luce (Aura of Light - Işık Hale) (1).mp3' dosyasındaki müziğin DNA analizini yap ve bana müzik türünü söyle"),
    ...
    ("L3-TOOL-19", "/Users/<user>/Local Documents/Suno Downloads/Aura di Luce (Aura of Light - Işık Hale) dizinindeki MP3 dosyalarının ID3 etiketlerini txt ve jpeg dosyalarını kullanarak otomatik doldur, TPE1 değerini 'Aura Artist' ve TALB değerini 'Aura Album' olarak ez (override et)"),
