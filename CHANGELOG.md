...
## Version 9 — 2026-09-05

**Full UBID coverage audit + block-count and tool-count correction (self-audit, prompted by an external question: "does the golden dataset actually cover every current tool?").**

- The private source repo's golden dataset had grown silently from 94 to 126 blocks: 32 new SUPP-TOOL/EK-TOOL-30..62 case-study blocks added the same day, closing every tool added 2026-07-25 through 2026-08-27, PheronAgent's new ConversationArchiveTool feature, and 12 older tools (`systemSleep`, `subagentDelegate`, `accessibility`, `chicagoVision`, `appDiscovery`, `contactsLookup`, `imageAnalysis`, `timerSet`, `semanticVision`, `skillPatch`, `notesTool`) that turned out to have never been covered even in the original 86/94-block battery — found only by auditing all 72 `ToolUBID` cases programmatically rather than trusting the existing "10 remaining gap" claim.
- This document had never been resynced for the private repo's earlier 68→86→94 growth either — the "58 core" figure was already one day stale on the day Version 8 itself was written (the private dataset had reached 94 blocks on 2026-07-31, the day before).
- **Corrected figures: 108 unique headline test blocks** (66 core, up from 58, + 42 case-study, SUPP-TOOL-20..62, up from 10); **72 tools**, correcting the "50+" figure used throughout (verified against `ToolUBID.allCases` in `ToolIDs.swift`).
- Found and recorded (Part VIII.4's new update note) that Part VII.4/VIII.4's own UBID-gap bookkeeping had become internally self-contradictory: VIII.4's own summary table already claimed "0 UBIDs left out of coverage," while the front matter and Part IV role box both still said "10 remain a genuine coverage gap," until this version. Several individual items in that gap list were also already wrong independent of this session's changes: UBID 21 (`calendarEvents`) no longer exists in the codebase at all; UBID 85 (`id3_processor`) and UBIDs 17/37 (`whatsappMessage`/`messengerMessage`) were already tested when the list was last written.
- Per this document's own established practice (Version 8), historical rows and Part VII.4's list itself are left as-written; corrections are recorded as new, dated notes alongside them, not silent rewrites.
- A full historical reconciliation of Part VII/VIII's UBID bookkeeping — which figure was right, when, and where each later one drifted — was explicitly **not** attempted; flagged as a known open follow-up rather than silently closed.
- Personal-info sanitization (`scripts/sanitize_and_publish.py`) was reviewed but not yet run for this dataset revision — the 126-block dataset still lives only in the private source repo pending its first live battery run (k=1 exploratory, per Section 2.6) and a publish pass.

## Version 8 — 2026-08-02

**Block-count correction (self-audit).** The "77 test blocks" figure was inflated: it double-counted 19 SUPP-TOOL-01..19 entries that the methodology document's own Section 13 already marked as "DUPLICATE, see L3-TOOL-xx" / "counted only under L3-TOOL, not run separately." A number cannot be both excluded from counting and included in the headline total.

- **Corrected figure: 68 unique test blocks** (58 core + 10 case-study blocks, SUPP-TOOL-20..29). SUPP-TOOL-01..19 are cross-references to Part IV.b's L3-TOOL-01..19, not additional blocks, and are no longer counted separately.
- All "77" references in `METHODOLOGY.md`/`METHODOLOGY_TR.md` (front matter, Part II role box, Part VIII summary tables, calibration section) corrected to 68. The 19 duplicate stub entries in Section 13 were consolidated into a single mapping table — no content lost, only repeated boilerplate removed.
- `STORY.md`'s account of the first full run was corrected: it had summarized the run as "all 77 defined test blocks" with a 44% pass rate, but the underlying result file (`run_20260701_1603.md`) itself contains a header/table mismatch (header says 71 of 77 tested; its own summary table sums to 80) — the narrative now states this discrepancy explicitly instead of picking one number silently. That run is also now explicitly labeled a k=1 exploratory pass (per Section 2.6), not a statistically meaningful pass rate.
- `STORY.md`'s bibliography claim ("61 sources, every one independently checked") was corrected to match `METHODOLOGY.md`'s own Part IX.2 statement: 60 of 61 were independently web-verified; 1 (Hermes Function-Calling Dataset) was carried over from the source document unverified, and is flagged as such rather than folded into the "verified" count.
- `LICENSE-docs.md`'s attribution example named "Turgay Soysal," inconsistent with the author name used everywhere else in the repo ("Turgay Savacı") — corrected.
- **Bibliography count was also stale:** Part IX.4's "conclusion" still said "40 total references / 39 verified" — a snapshot from before Version 6 added 27 more citations to Part IX.2 (34→61). Corrected to the current total: **67 references** (61 in IX.2 + 6 in IX.3), **66 independently verified**, with the Hermes Function-Calling Dataset remaining the one honestly-flagged exception (as it always was). The original itemized verification list from the earlier pass was kept for traceability, not deleted.
- Historical version rows above (Version 2, Version 6) are left as originally written — they are a dated record of what was believed true at the time, not restated as current fact.
