...
With the protocol corrected and the harness stabilized, the first broad run against the test battery happened: 80 test instances attempted (the run file's own header says "71 of 77 canonical blocks," while its summary table sums to 80 — a discrepancy in the source file itself, left as-is rather than silently reconciled here). The result: roughly **44% pass** (35 pass / 38 fail / 3 skip / 4 partial), recorded in `run_20260701_1603.md`. This was a k=1 exploratory pass under the document's own later-added Section 2.6 rule — indicative of where the bugs were, not a statistically meaningful pass rate (that requires k≥5, see Chapter 6).
...
`AgentTestMethodology` is the public extraction of that entire process. `METHODOLOGY_TR.md` is the document in Turkish, the language the whole process above actually happened in; `METHODOLOGY.md` is its English counterpart. The `templates/` directory, the dual CC BY 4.0 / MIT license, and this file all trace directly back to the five-gap self-audit in Chapter 6.
...
That same discipline applied to producing `METHODOLOGY.md` itself. It wasn't hand-translated section by section and trusted; it was checked the way everything else in this document gets checked — by counting. A heading-level and table-row diff against the Turkish original turned up two benchmark write-ups (VisualWebArena, AssistantBench) that a translation pass had silently dropped, plus one section (L4-YÜK-01) that had been duplicated across a chunk boundary — one real gap, one real duplicate, found by comparing structure rather than trusting that a clean-looking output meant a complete one. Both were fixed before publishing.
